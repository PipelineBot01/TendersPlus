...
from conf.file_path import RESEARCHER_TAG_CATE_MAP_PATH, RESEARCHER_DIVISION_MAP_PATH
...
class TendersIter:
    def __init__(self,
                 tag_div_map_path=RESEARCHER_TAG_CATE_MAP_PATH,
                 researcher_div_map_path=RESEARCHER_DIVISION_MAP_PATH):
    ...
    def __update_divs(self, new_tag_df: pd.DataFrame):
        tmp_df = self.__user_div_map.groupby('division')['Staff ID'].count().reset_index().rename(
            columns={'Staff ID': 'cnt'})
...
ti = TendersIter('../assets/tag_division_map.csv', '../assets/researcher_division.csv')
