...
class ResearcherIter:
    ...
    def fit_list(self, div_list: List[str], tag_list: List[str]) -> pd.DataFrame:
        ...
        tmp_df = tmp_df.merge(
            tmp_df.groupby(self.pk)['division'].count().reset_index().rename(columns={'division': 'cnt'}))
        tmp_df['weight'] = 1 / tmp_df['cnt']
        tag_div_map_df = self.__update_divs(tmp_df)
...
if __name__ == '__main__':
    ...
    ti.fit_list(['d_11', 'd_12', 'd_13'], ['ontology'] * 90)
