...
class Scraper:
    ...
    def save2Mongo(self):
        ...
        print('******* start update collection "open" *********')
        mongodb.delete({})

        result = mongodb.insert(self.tenders)
        print('******* finish update collection "open" *********')

        mongodb.use_database('all')
        urls = mongodb.find_col("ATM ID")
        df = pd.DataFrame(urls)

        open_df = pd.DataFrame(self.tenders)
        all_fields = ['ATM ID'
            , 'Title'
            , 'Close Date & Time'
            , 'Agency'
            , 'Category'
            , 'Description']
        open = open_df[all_fields]
        tender_json = json.loads(open.to_json(orient="records"))
        print("******start update collection 'all'******")

        for t in tender_json:
            if len(df) != 0:
                skipslist = str(list(df["ATM ID"]))
                if t["ATM ID"] in skipslist:
                    continue
            r = mongodb.insert(t)
        print("******finish update collection 'all'******")
