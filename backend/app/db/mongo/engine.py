...
async def init_db() -> None:
    ...
    mongo['tenders_client'].get_default_database().get_collection('clean_grants_opened').create_index(
        [('title', 'text'), ('tags', 'text')], name='clean_grants_opened_text_index')

    # count the records
    mongo['tenders_client_docs_count'] = {}
    mongo['tenders_client_docs_count']['clean_grants_opened'] = await mongo['tenders_client'].get_default_database().get_collection('clean_grants_opened').count_documents({})
